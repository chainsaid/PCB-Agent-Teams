...
# ---------------------------------------------------------------------------
# Airflow / convection correction on Rθ_JA
# ---------------------------------------------------------------------------
# JEDEC Rθ_JA values (the package table below) are measured in *still air*
# (natural convection, JESD51-2). Forced air lowers Rθ_JA because the
# board+package shed more heat per °C of rise. The reduction is monotonic
# in air velocity and *saturates* — past ~1000 LFM the convection film
# coefficient gains little.
#
# We model the multiplier applied to Rθ_JA as a smooth, bounded curve:
#
#     k(v) = K_MIN + (1 - K_MIN) / (1 + (v / V_HALF))
#
#   v       air velocity in LFM (linear feet per minute)
#   K_MIN   asymptotic floor of the multiplier at very high airflow
#   V_HALF  velocity at which half of the available reduction is reached
#
# k(0) == 1.0 exactly  → still-air default is byte-identical to prior runs.
# The shape and the named-tier anchors below are engineering approximations
# drawn from typical JEDEC convection curves / vendor thermal app notes
# (order-of-magnitude reductions: ~20% near 200 LFM, ~35% near 500 LFM,
# ~45% near 1000 LFM). They are first-order, not a substitute for CFD.
AIRFLOW_K_MIN = 0.50    # floor: forced air can roughly halve Rθ_JA, not more
AIRFLOW_V_HALF = 600.0  # LFM at which half the available reduction is reached

# Named airflow tiers → representative LFM. Users may also pass a raw number.
AIRFLOW_TIERS = {
    "still": 0.0,         # JEDEC still-air baseline (== default behavior)
    "natural": 0.0,       # natural convection == JEDEC still-air baseline
    "low": 100.0,         # gentle / incidental airflow
    "forced": 250.0,      # typical small board-level fan
    "high": 500.0,        # strong forced air
}

# ---------------------------------------------------------------------------
# Inter-component thermal coupling (first-order superposition)
# ---------------------------------------------------------------------------
# Neighboring heat sources raise a component's junction temperature beyond
# its own self-heating. We add a first-order superposition term: each
# nearby dissipator contributes a temperature rise that decays with
# distance. This is a coarse lumped estimate (point sources on a uniform
# spreading plane), NOT a finite-element / CFD result.
#
#     ΔT_couple(self) = Σ_neighbors  P_neighbor · COUPLE_GAIN
#                                     · D_REF / (D_REF + dist_mm)
#
# The D_REF / (D_REF + d) factor is 1.0 at zero separation and falls toward
# 0 as parts move apart — a bounded, monotonic stand-in for radial heat
# spreading. COUPLE_GAIN sets how many °C of neighbor-induced rise a 1 W
# adjacent source produces at close range; it is a conservative engineering
# constant, not a measured value.
COUPLE_PROXIMITY_MM = 10.0  # neighbors beyond this are ignored (matches the
                            # existing thermal-proximity warning radius)
COUPLE_GAIN = 12.0          # °C per adjacent watt at close range (1st-order)
COUPLE_D_REF = 5.0          # mm; distance scale of the spreading falloff
...
# ---------------------------------------------------------------------------
# Airflow / convection helpers
# ---------------------------------------------------------------------------

def _resolve_airflow_lfm(airflow) -> float:
    """Resolve an --airflow argument to an air velocity in LFM.

    Accepts a named tier ('still'/'natural'/'low'/'forced'/'high') or a raw
    numeric value (LFM). Returns 0.0 (still air) for None or anything
    unrecognized — so the default path stays byte-identical to prior runs.
    """
    if airflow is None:
        return 0.0
    if isinstance(airflow, (int, float)):
        return max(0.0, float(airflow))
    s = str(airflow).strip().lower()
    if s in AIRFLOW_TIERS:
        return AIRFLOW_TIERS[s]
    try:
...
def _airflow_factor(lfm: float) -> float:
    """Multiplier applied to Rθ_JA for a given air velocity (LFM).

    k(0) == 1.0 exactly (still-air baseline). Monotonically decreasing and
    bounded below by AIRFLOW_K_MIN. See the module-level model notes.
    """
    if lfm <= 0.0:
        return 1.0
    return AIRFLOW_K_MIN + (1.0 - AIRFLOW_K_MIN) / (1.0 + (lfm / AIRFLOW_V_HALF))
...
def _compute_junction_temps(power_comps: list, pcb: dict,
                            extract_dir: str, ambient_c: float,
                            airflow_lfm: float = 0.0) -> list:
    """Compute estimated junction temperature for each power component.

    airflow_lfm scales every package Rθ_JA by the convection factor (see
    _airflow_factor). 0.0 == still air == prior behavior (factor 1.0).
    """
    airflow_factor = _airflow_factor(airflow_lfm)
    ...
    for comp in power_comps:
        ...
        # PCB correction (copper / thermal vias) then airflow convection.
        # airflow_factor == 1.0 in still air, so the still-air result is
        ...
        # Junction temperature: Tj = Ta + P × Rθ_JA (self-heating only;
        # neighbor coupling is added in a second pass below).
        pdiss = comp["pdiss_w"]
        ...
        assessments.append({
            ...
            "airflow_factor": round(airflow_factor, 3),
    ...
    # Second pass: first-order inter-component thermal coupling.
    # Reported as *additive metadata* — tj_estimated_c (self-heating),
    # margin_c, findings, score and sort order are all left untouched so
    # that default output is byte-identical to prior versions. The
    # coupling-aware estimate is exposed separately as tj_with_coupling_c.
    _apply_thermal_coupling(assessments)

    # Sort by self-heating Tj descending (hottest first) — unchanged.
    assessments.sort(key=lambda a: -a["tj_estimated_c"])
    return assessments


def _apply_thermal_coupling(assessments: list) -> None:
    """Annotate each assessment with first-order neighbor heat coupling.

    For every component with a known position, sum the distance-weighted
    dissipation of all *other* positioned power components within
    COUPLE_PROXIMITY_MM. The result is exposed as additive metadata:

        tj_coupling_rise_c   °C added by neighbors (>= 0)
        tj_with_coupling_c   tj_estimated_c + rise (conservative estimate)
        margin_with_coupling_c
        coupling_neighbors   [{ref, dist_mm, delta_c}]

    tj_estimated_c / margin_c are NOT modified, so existing findings and
    scoring are unaffected. First-order lumped estimate only — not FEA/CFD.
    """
    positioned = [a for a in assessments
                  if a.get("position") and a.get("pdiss_w", 0) > 0]
    for a in assessments:
        ...
        if pos:
            ...
            for other in positioned:
                ...
                if dist > COUPLE_PROXIMITY_MM:
                    continue
                weight = COUPLE_D_REF / (COUPLE_D_REF + dist)
                contrib = other["pdiss_w"] * COUPLE_GAIN * weight
                if contrib <= 0:
                    continue
                rise += contrib
                neighbors.append({
                    ...
                    "delta_c": round(contrib, 2),
                })

        a["tj_coupling_rise_c"] = round(rise, 1)
        a["tj_with_coupling_c"] = round(a["tj_estimated_c"] + rise, 1)
        a["margin_with_coupling_c"] = round(
            a["tj_max_c"] - a["tj_with_coupling_c"], 1)
        a["coupling_neighbors"] = neighbors
...
def _board_summary(assessments: list, ambient_c: float,
                   airflow_lfm: float = 0.0) -> dict:
    ...
    if not assessments:
        return {
            ...
            "ambient_c": ambient_c,
            "airflow_lfm": round(airflow_lfm, 1),
            "airflow_factor": round(_airflow_factor(airflow_lfm), 3),
        }
...
def format_text_report(result: dict) -> str:
    ...
    airflow_factor = summary.get("airflow_factor", 1.0)
    if airflow_factor < 1.0:
        lfm = summary.get("airflow_lfm", 0)
        lines.append(f"Airflow:         {lfm:.0f} LFM "
                     f"(Rθ_JA ×{airflow_factor:.2f})")
    ...
    # Inter-component thermal coupling (first-order). Source the TH-DET
    # entries from findings[] (assessments are merged in before output).
    # Only emitted when at least one component sees neighbor coupling, so
    # boards without dense heat sources keep their prior output unchanged.
    coupled = [f for f in findings
               if isinstance(f, dict)
               and f.get("rule_id") == "TH-DET"
               and f.get("tj_coupling_rise_c", 0) > 0]
    if coupled:
        coupled.sort(key=lambda f: -f.get("tj_coupling_rise_c", 0))
        lines.append("-" * 60)
        lines.append("Inter-Component Thermal Coupling (first-order, +ΔTj)")
        lines.append("-" * 60)
        for f in coupled:
            rise = f.get("tj_coupling_rise_c", 0)
            tj_c = f.get("tj_with_coupling_c", 0)
            margin_c = f.get("margin_with_coupling_c", 0)
            srcs = ", ".join(
                f"{n['ref']}(+{n['delta_c']:.1f}°C @{n['dist_mm']:.1f}mm)"
                for n in f.get("coupling_neighbors", []))
            lines.append(
                f"  {f.get('ref', '?'):<8} +{rise:.1f}°C -> Tj≈{tj_c:.0f}°C "
                f"(margin {margin_c:.0f}°C)")
            if srcs:
                lines.append(f"      from: {srcs}")
        lines.append("")
    ...
    if findings:
        ...
        for f in findings:
            ...
            # TH-DET assessment entries carry `summary` but no `title`;
            # fall back so the text report never crashes on merged entries.
            title = f.get("title") or f.get("summary", "")
            lines.append(f"  [{sev}] {f.get('rule_id', '?')}: {title}")
...
def main():
    ...
    parser.add_argument("--airflow", default=None,
                        help="Forced-air cooling: a named tier "
                             "(still/natural/low/forced/high) or a raw LFM "
                             "number. Lowers Rθ_JA via a convection factor. "
                             "Default (omit) = still air, identical to prior "
                             "behavior.")
    ...
    if args.schema:
        schema = {
            ...
            "findings": "[{detector, rule_id, category, severity, confidence, evidence_source, summary, description, components, nets, pins, recommendation, report_context}] — TS-001..005, TP-001..002, TH-DET assessments. TH-DET entries also carry airflow_factor, tj_coupling_rise_c, tj_with_coupling_c, margin_with_coupling_c, coupling_neighbors (first-order, additive — tj_estimated_c stays self-heating only)",
            "trust_summary": {
                "total_findings": "int",
    ...
    # Airflow: CLI wins; otherwise fall back to project config; otherwise
    # still air (None). project may carry "airflow" as a tier or LFM number.
    if args.airflow is None and project.get("airflow") is not None:
        args.airflow = project["airflow"]
    airflow_lfm = _resolve_airflow_lfm(args.airflow)
    ...
    assessments = _compute_junction_temps(
        power_comps, pcb, extract_dir, args.ambient, airflow_lfm)
    ...
    # Board summary
    board = _board_summary(assessments, args.ambient, airflow_lfm)
