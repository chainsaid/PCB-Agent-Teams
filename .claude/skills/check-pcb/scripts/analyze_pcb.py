...
def extract_setup(root: list) -> dict:
    ...
    # Board-level via tenting default — KiCad 9 stores it as
    #   (setup (tenting (front yes|no) (back yes|no)))
    # Per-via overrides are read separately into via_info["tenting"].
    tenting_node = find_first(setup_node, "tenting")
    if tenting_node:
        result["tenting_default"] = {
            "front": get_value(tenting_node, "front") == "yes",
            "back": get_value(tenting_node, "back") == "yes",
        }
...
def analyze_thermal_pad_vias(footprints: list[dict], vias: dict,
                             zones: list[dict],
                             zone_fills: "ZoneFills",
                             board_tenting: dict | None = None) -> list[dict]:
        ...
        for pad in thermal_pads_found:
            ...
            for via in all_vias:
                ...
                if (abs(dx) <= half_w * 1.5 and
                    ...
                    if is_via_tented(via, board_tenting):
                        vias_tented += 1
                    else:
                        vias_untented += 1
...
_SILK_UNIT_RE = re.compile(
    r'(?<![A-Za-z0-9.])(\d+(?:\.\d+)?)\s*'
    r'(GHz|MHz|kHz|Hz|kVrms|kV|mV|V|mA|kA|A|mΩ|Ω|pF|nF|uF|µF|mF|F|ppm|%)'
    r'(?![A-Za-z0-9])',
    re.IGNORECASE,
)


def _norm_silk_unit(u: str) -> str:
    return u.replace("µ", "u").lower()


def _norm_silk_value(num_str: str, unit: str) -> tuple:
    try:
        v = float(num_str)
    except ValueError:
        return (num_str, unit)
    return (v, _norm_silk_unit(unit))


def _find_project_claude_md(pcb_path: str) -> Path | None:
    """Walk up from a .kicad_pcb to the closest Projects/<name>/CLAUDE.md."""
    p = Path(pcb_path).resolve().parent
    for _ in range(6):
        candidate = p / "CLAUDE.md"
        if candidate.is_file() and "Projects" in p.parts:
            return candidate
        if p.parent == p:
            break
        p = p.parent
...
def analyze_silk_spec_consistency(silkscreen: dict, pcb_path: str) -> list[dict]:
    """SK-002: silk text values that don't appear in the project CLAUDE.md spec.

    Background: users hand-write silk like 'Voltage Sensor 400V Fc 100kHz' as a
    hard-coded echo of design parameters. When the spec changes (fc 20→100 kHz)
    the silk silently goes stale, and no DRC / EMC check would notice — physical
    boards get printed with the wrong number. This pass parses the closest
    Projects/<name>/CLAUDE.md, extracts every (number, unit) token, and warns
    when silk has a unit-bearing value that has no match in the spec text.

    Heuristic, not authoritative: catches drift (the common case) but a silk
    value that legitimately doesn't appear in CLAUDE.md (e.g. board version
    'V1.2') will false-warn. Fix the silk or add the value to the spec table.
    """
    findings: list[dict] = []
    claude_md = _find_project_claude_md(pcb_path)
    if not claude_md:
        return findings
    try:
        spec_text = claude_md.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return findings

    spec_tokens = {
        _norm_silk_value(m.group(1), m.group(2))
        for m in _SILK_UNIT_RE.finditer(spec_text)
    }
    if not spec_tokens:
        return findings

    silk_entries: list[tuple[str, str]] = []
    for t in silkscreen.get("board_texts", []):
        silk_entries.append((t.get("text", ""), t.get("layer", "")))
    for t in silkscreen.get("user_silk_texts", []):
        silk_entries.append((t.get("text", ""), "(footprint silk)"))

    for text, layer in silk_entries:
        if not text:
            continue
        for m in _SILK_UNIT_RE.finditer(text):
            tok = _norm_silk_value(m.group(1), m.group(2))
            if tok not in spec_tokens:
                findings.append({
                    "rule_id": "SK-002",
                    ...
                    "value": m.group(0),
                    "layer": layer,
                    "spec_file": str(claude_md),
                    "summary": f"Silk text {text!r} has '{m.group(0)}' not echoed in CLAUDE.md",
                    "description": (
                        f"Silkscreen text {text!r} contains the parameter '{m.group(0)}', but "
                        f"no matching value (number+unit) was found in {claude_md.name}. "
                        f"Likely a stale design-parameter echo from before a spec change. "
                        f"Verify against §技术参数表; if the board param really changed, "
                        f"refresh the silk text. If silk is correct, add the value to CLAUDE.md."
                    ),
                })
    return findings
...
def is_via_tented(via_info: dict, board_default: dict | None) -> bool:
    """True if the via has solder mask coverage on at least one side.

    KiCad 9 sources of truth:
      - Per-via override: ``(via ... (tenting front back))`` or ``(tenting none)``.
        Stored on via_info as the list ``["front", "back"]`` / ``["none"]``.
      - Board default: ``(setup (tenting (front yes) (back yes)))`` →
        ``setup["tenting_default"] = {"front": True, "back": True}``.

    Per-via overrides win when present. "Tented" here means at least one side
    has mask — either side closed prevents wick-through during reflow.
    """
    override = via_info.get("tenting")
    if override:
        return any(side in ("front", "back") for side in override)
    if not board_default:
        # KiCad 7/8 default is tented unless explicitly disabled via setup.
        return True
    return bool(board_default.get("front") or board_default.get("back"))


def analyze_via_in_pad(footprints: list[dict], vias: dict, thermal_pad_refs: set,
                       board_tenting: dict | None = None) -> list[dict]:
    ...
    for via in via_list:
        ...
        for ref, pad_num, net, x1, y1, x2, y2 in smd_pads:
            if x1 <= vx <= x2 and y1 <= vy <= y2:
                tented = is_via_tented(via, board_tenting)
...
def analyze_pcb(path: str, *, proximity: bool = False,
    ...
    thermal_pad_vias = analyze_thermal_pad_vias(
        footprints, vias, zones, zone_fills,
        board_tenting=setup.get("tenting_default"))
    ...
    # SK-002: silk values that don't appear in project CLAUDE.md (stale spec echo).
    silk_spec_drift = analyze_silk_spec_consistency(silkscreen, str(path))
    if silk_spec_drift:
        result["silk_spec_drift"] = silk_spec_drift
        # also surface in silkscreen.documentation_warnings for back-compat readers
        result.setdefault("silkscreen", silkscreen).setdefault(
            "documentation_warnings", []
        ).extend({
            "type": "silk_value_uncorroborated",
            "severity": f["severity"],
            "rule_id": f["rule_id"],
            "silk_text": f["silk_text"],
            "value": f["value"],
            "message": f["summary"],
        } for f in silk_spec_drift)
