...
def check_thermal_emc(pcb: Optional[Dict],
                      schematic: Optional[Dict],
                      thermal: Optional[Dict] = None) -> List[Dict]:
    ...
    When `thermal` (parsed analyze_thermal.py output) is provided, TH-002
    severity is driven by the *actual* estimated junction temperature of the
    regulator (Tj_c) instead of the proximity-only heuristic. Regulators
    below 60 °C are skipped (cool enough that ferrite µ is barely affected),
    while ≥100 °C escalates from LOW to HIGH and confidence to deterministic.

    Ref: Analog Devices, "Temperature and Voltage Variation of Ceramic
    Capacitors"; Murata DC bias characteristic documentation.
    """
    # Build {ref: {tj_c, tj_max_c, margin_c}} lookup from TH-DET findings
    thermal_by_ref: Dict[str, Dict] = {}
    if thermal:
        for tf in thermal.get('findings', []):
            if not isinstance(tf, dict) or tf.get('rule_id') != 'TH-DET':
                continue
            ref = tf.get('ref') or (tf.get('components') or [None])[0]
            if not ref:
                continue
            thermal_by_ref[ref] = {
                'tj_c': tf.get('tj_estimated_c'),
                'tj_max_c': tf.get('tj_max_c'),
                'margin_c': tf.get('margin_c'),
            }
    ...
    if pcb:
        ...
        # Find switching regulator positions. When thermal data is
        # available, also stash the estimated Tj so the proximity check
        # can promote severity by actual heat instead of "any switcher
        # is hot".
        reg_positions = []
        for reg in regulators:
            if reg.get('topology', '').lower() in ('ldo', 'linear'):
                continue
            ref = reg.get('ref', reg.get('reference', ''))
            tj = thermal_by_ref.get(ref, {}).get('tj_c') if thermal_by_ref else None
            # Skip regulators we know are cool (< 60 °C) — ferrite µ
            # barely shifts at those temps, so the proximity warning
            # would be noise.
            if thermal_by_ref and isinstance(tj, (int, float)) and tj < 60:
                continue
            for fp in footprints:
            ...
            for reg_pos in reg_positions:
                ...
                if dist <= 10.0:
                    ...
                    if isinstance(tj, (int, float)):
                        # Severity ladder driven by actual Tj from
                        # analyze_thermal: 60-85 °C stays LOW, 85-100 °C
                        # bumps to MEDIUM (µ drop becomes measurable),
                        # ≥100 °C HIGH (30-50 % impedance loss territory).
                        if tj >= 100:
                            severity = 'HIGH'
                        elif tj >= 85:
                            severity = 'MEDIUM'
                            ...
                            severity = 'LOW'
                        thermal_note = (
                            f'Regulator {reg_pos["ref"]} estimated '
                            f'Tj={tj:.0f}°C (from thermal analysis).'
                        )
                        confidence = 'deterministic'
                    else:
                        severity = 'LOW'
                        thermal_note = (
                            'Regulator Tj unknown — pass --thermal '
                            'thermal.json to drive severity by actual '
                            'junction temperature.'
                        )
                        confidence = 'datasheet-backed'

                    findings.append(_make_finding(
                        'thermal_emc', severity, 'TH-002',
                        title=f'Ferrite {ref} near switching regulator {reg_pos["ref"]}',
                        description=(
                            f'{ref} ({fp.get("value","")}) is {dist:.1f}mm from '
                            f'switching regulator {reg_pos["ref"]}. {thermal_note} '
                            f'Ferrite permeability decreases with temperature — '
                            f'above 85°C the impedance can drop 30-50%, '
                            f'reducing filtering.'
                        ),
...
def run_all_checks(schematic: Optional[Dict], pcb: Optional[Dict],
    ...
    Args:
        ...
        thermal: analyze_thermal.py output JSON (optional). When provided,
                 TH-002 escalates by actual estimated junction temperature
                 instead of "any non-LDO regulator is assumed hot".
    ...
    if schematic and pcb:
        ...
        all_findings.extend(check_thermal_emc(pcb, schematic, thermal=thermal))
